...
describe Monga::Database do
  before do
    EM.run do
      @client = Monga::Client.new(type: :em, pool_size: 2, servers: ["188.93.61.42:27017", "188.93.61.42:27018", "188.93.61.42:27019"])
      @db = @client["dbTest"]
    ...
    it "should get last error" do
      EM.run do
        @client.aquire_connection do |connection|
          options = {}
          options[:documents] = { name: "Peter", personal_id: 10 }
          Monga::Protocol::Insert.new(connection, @db.name, @collection.collection_name, options).perform
          @db.get_last_error(connection) do |err, resp|
    ...
    it "should getLastError with fsync" do
      EM.run do
        @client.aquire_connection do |connection|
          ...
          options[:documents] = { name: "Peter", personal_id: 10 }
          Monga::Protocol::Insert.new(connection, @db.name, @collection.collection_name, options).perform
          @db.get_last_error(connection, fsync: true) do |err, resp|
            resp["ok"].must_equal 1.0
            Monga::Protocol::Insert.new(connection, @db.name, @collection.collection_name, options).perform
            @db.get_last_error(connection, fsync: true) do |err, resp|
              err.class.must_equal Monga::Exceptions::QueryFailure
